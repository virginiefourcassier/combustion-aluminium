(() => {
  ...
  function reactionProbability(){
    // Probabilité élevée dès le début pour réduire la durée totale (< ~1 min avec réglages par défaut)
    const tf = tempFactor(); // 1..10
    return Math.min(0.97, 0.28 + 0.07 * tf); // 0.35..0.98 (cap)
  }
...
function findNearbyO2(x, y, radius){
  ...
  function tryReactionAt(o2){
    ...
    const nearAl = findNearbyAl(o2.x, o2.y, 42);
    if(nearAl.length < 1) return;

    // Réaction si on dispose de 4 Al ET 3 O2 proches
    const nearO2 = findNearbyO2(o2.x, o2.y, 62);
