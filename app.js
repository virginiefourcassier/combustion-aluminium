(() => {
  ...
  function generateAlPile(count){
    ...
    while(remaining>0){
      const i = rows + Math.floor(Math.random()*3);
    ...
    // sécurité : tout visible
    aluminium.forEach(a => {
      a.x = Math.max(a.r+6, Math.min(WIDTH - a.r - 6, a.x));
      a.y = Math.max(a.r+6, Math.min(GROUND - a.r - 2, a.y));
    });
  }
  ...
  function reactionProbability(){
    ...
    return Math.min(0.70, 0.08 * tf); // 0.08..0.40 (cap à 0.70)
  }
