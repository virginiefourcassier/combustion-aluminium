(() => {
  ...
  function generateO2function generateO2(count){
    dioxygen = [];
    const r = 10;
  ...
  function reactionProbability(){
    // Probabilité très élevée dès le début pour finir en < 1 min (réglages par défaut)
    const tf = tempFactor(); // 1..10
    return Math.min(0.98, 0.60 + 0.04 * tf); // 0.64..0.98
  }
...
function findNearbyO2(x, y, radius){
  ...
  function tryReactionAt(o2){
    // Réaction au contact du solide : on exige 4 Al proches,
    // et on consomme 3 O2 (pris parmi les plus proches disponibles) pour garantir la formation de produit.
    if(o2.used) return;

    const nearAl = findNearbyAl(o2.x, o2.y, 46);
    if(nearAl.length < 4) return;

    // il faut au moins 3 O2 disponibles au total
    const o2Avail = dioxygen.filter(x=>!x.used).length;
    if(o2Avail < 3) return;

    if(Math.random() < reactionProbability()){
      // consommer 4 Al externes en priorité
      nearAl.sort((p,q)=>exposureScore(p)-exposureScore(q));
      for(let i=0;i<4;i++) nearAl[i].used = true;

      // consommer 3 O2 parmi les plus proches du point d'impact
      const picked = takeNearestUnusedO2(o2.x, o2.y, 3);
